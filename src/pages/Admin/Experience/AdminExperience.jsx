...
const AdminExperience = () => {
  ...
  const handleSave = async (values) => {
    ...
    try {
      if (editing) {
        await experienceService.update(editing._id, payload);
  ...
  const handleDelete = async (id) => { await experienceService.delete(id); refetch(); };

  const columns = [
    { title: 'Position', dataIndex: 'position', key: 'position', render: v => <span style={{ color: 'var(--color-heading)', fontWeight: 600 }}>{v}</span> },
  ...
  return (
    <>
      <CrudTable title="Experience" data={data} loading={loading} columns={columns} onAdd={openAdd} onEdit={openEdit} onDelete={handleDelete} />
      <Modal
        open={modalOpen} onCancel={() => setModalOpen(false)}
        title={<span style={{ color: 'var(--color-heading)' }}>{editing ? 'Edit Experience' : 'Add Experience'}</span>}
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleSave} style={{ marginTop: 16 }}>
          <Form.Item name="position" label={<span style={{ color: 'var(--color-text-muted)' }}>Position</span>} rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          <Form.Item name="company" label={<span style={{ color: 'var(--color-text-muted)' }}>Company</span>} rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <Form.Item name="endDate" label={<span style={{ color: 'var(--color-text-muted)' }}>End Date (leave blank = Present)</span>}>
              <DatePicker picker="month" style={{ width: '100%' }} />
            </Form.Item>
          </div>
