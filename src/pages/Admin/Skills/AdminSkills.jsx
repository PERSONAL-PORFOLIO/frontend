...
const catColors = { Frontend: 'blue', Backend: 'cyan', Database: 'green', Mobile: 'orange', DevOps: 'red', Other: 'purple' };
...
const AdminSkills = () => {
  ...
  const handleSave = async (values) => {
    setSaving(true);
    try {
      const payload = { ...values, iconUrl: iconUrl || null };
      if (editing) {
  ...
  const columns = [
    ...
    { title: 'Category', dataIndex: 'category', key: 'category', render: v => <Tag color={catColors[v] || 'default'}>{v}</Tag> },
    { title: 'Level', dataIndex: 'level', key: 'level', render: v => <span style={{ color: 'var(--color-primary-light)' }}>{v}%</span> },
  ];

  return (
    <>
      <CrudTable title="Skills" data={data} loading={loading} columns={columns} onAdd={openAdd} onEdit={openEdit} onDelete={handleDelete} />
      <Modal
        open={modalOpen}
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleSave} style={{ marginTop: 16 }}>
          ...
          <Form.Item name="category" label={<span style={{ color: 'var(--color-text-muted)' }}>Category</span>} rules={[{ required: true }]}>
          ...
          {/* Icon: upload image/SVG + emoji fallback */}
          <Form.Item label={<span style={{ color: 'var(--color-text-muted)' }}>Icon</span>}>
            ...
            {iconUrl && (
              <div style={{ marginTop: 8, fontSize: '0.75rem', color: '#10b981' }}>
                Uploaded image will be shown on the public site.
              </div>
            )}
          </Form.Item>
